...
class Tournament extends Component {
  ...
  componentDidMount() {
    fetch(API + "/" + this.props.id)
      ...
      .then(data => this.setState({tournament: data.tournament, ready: true}));
  }
  ...
  render() {
    ...
    return (
      this.state.ready ?
        <div className="container" id="tournament-list">
          <h1>{tm.name || "Loading..."}</h1>
                ...
                <div className="card mx-1" key={i}>
                  ...
                  <div className="card-body">
                    <ul className="list-group list-group-flush">
                      {group.map((team, j) =>
                        <li className="list-group-item text-center" key={j}>{team[0]} & {team[1]}</li>
                      )}
                    </ul>
                  </div>
                </div>
              ...
              {tm.matchesSelection.map((group, i) =>
                <div className="card mx-1" key={i}>
                  ...
                  <div className="card-body">
                    <ul className="list-group list-group-flush">
                      {group.map((team, j) =>
                        <li className="list-group-item text-center" key={j}><span className="text-primary">{team[0].join(' & ')}</span> Vs <span className="text-danger">{team[1].join(' & ')}</span></li>
                      )}
                    </ul>
                  </div>
                </div>
